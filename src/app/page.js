    ...
    <div className="flex flex-col w-full">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 w-full">
        ...
        <div className="flex justify-between items-end mb-8 border-b border-[#1A1A1A]/10 pb-4">
          <div className="flex items-baseline gap-6">
            <h2 className="text-3xl font-serif tracking-tight text-[#1A1A1A] italic">NEW ARRIVALS</h2>
            <a href="/catalogue" className="text-xs font-semibold tracking-widest uppercase text-[#1A1A1A]/70 hover:text-[#1A1A1A]">
              Shop All
            </a>
          </div>
          <div className="flex gap-2 text-[#1A1A1A]/40">
            <button className="hover:text-[#1A1A1A] transition-colors" aria-label="Previous">
            ...
            <button className="hover:text-[#1A1A1A] transition-colors" aria-label="Next">
      ...
      <section className="w-full flex flex-col items-center">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="flex justify-between items-end mb-10 border-b border-[#1A1A1A]/10 pb-6">
            <div className="flex items-baseline gap-6 w-full justify-center">
              <h2 className="text-5xl md:text-7xl font-serif tracking-tight text-[#1A1A1A] italic">PINTEREST COLLECTION</h2>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 bg-[#1A1A1A] text-[#F8F5F1] overflow-hidden w-full">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col w-full">
            {['Co-Ords', 'Dresses', 'Knitwear', 'Accessories'].map((category, idx) => (
              <div 
                key={idx}
                className="border-b border-[#F8F5F1]/20 py-8 md:py-12 group cursor-pointer flex justify-between items-center"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-4 w-full h-[150vh] md:h-[90vh]">
          {[
            ...
            <Link 
              key={idx} 
              href={item.link}
              className="relative w-full h-full overflow-hidden group cursor-pointer bg-[#F8F5F1] rounded-2xl md:rounded-3xl"
            >
              <img 
                src={item.img} 
                alt={item.title} 
      ...
      <section className="py-20 bg-white w-full">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 mb-8 text-center">
          <h2 className="text-2xl md:text-3xl font-sans font-bold tracking-tight text-[#1A1A1A] mb-2">Curated Aesthetics</h2>
          <p className="text-xs md:text-sm text-[#1A1A1A]/60 tracking-widest uppercase mb-8">OUR CUTEST PICKS,JUST FOR YOU 💕</p>
          
          {/* Pills / Filters */}
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 w-full">
            <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar w-full md:w-auto pb-2 md:pb-0">
              <button className="bg-[#1A1A1A] text-white px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap">Best Sellers</button>
              <button className="border border-[#1A1A1A] text-[#1A1A1A] px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap hover:bg-[#1A1A1A]/5 transition-colors">New Arrivals</button>
              <button className="border border-[#1A1A1A] text-[#1A1A1A] px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap hover:bg-[#1A1A1A]/5 transition-colors">Sets</button>
              <button className="border border-[#1A1A1A] text-[#1A1A1A] px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap hover:bg-[#1A1A1A]/5 transition-colors">Dresses</button>
              <button className="border border-[#1A1A1A] text-[#1A1A1A] px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap hover:bg-[#1A1A1A]/5 transition-colors">Tops</button>
              <button className="border border-[#1A1A1A] text-[#1A1A1A] px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap hover:bg-[#1A1A1A]/5 transition-colors">Outerwear</button>
            </div>
            <button className="border border-[#1A1A1A] text-[#1A1A1A] px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap hover:bg-[#1A1A1A]/5 transition-colors hidden md:block">Shop Best Sellers</button>
          </div>
        </div>

        {/* Seamless Grid */}
        <div className="w-full border-t border-b border-[#1A1A1A]/20">
          <div className="grid grid-cols-2 md:grid-cols-5 w-full">
            {curatedItems.map((item, idx) => (
              <Link href={`/product/${item.id}`} key={item.id || idx} className="border-r border-[#1A1A1A]/20 group cursor-pointer flex flex-col relative bg-white block">
                {/* Image Block */}
                <div className="relative aspect-[3/4] w-full overflow-hidden bg-[#F8F5F1]">
                  ...
                  {idx < 2 && (
                    <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-[11px] font-semibold text-[#1A1A1A]">
                      Selling Fast
                    </div>
                  )}
                  ...
                  <div className="absolute bottom-0 left-0 w-full bg-white/80 backdrop-blur-md p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex flex-col items-center">
                    <p className="text-[10px] font-bold tracking-widest mb-2 uppercase text-[#1A1A1A]">Select a Size</p>
                    <div className="flex gap-1.5 justify-center flex-wrap">
                      {(item.sizes || [4,6,8,10,12,14,16]).map(s => (
                        <button key={s} className="w-6 h-6 border border-[#1A1A1A]/20 bg-white text-[10px] font-medium flex items-center justify-center hover:border-[#1A1A1A] transition-colors text-[#1A1A1A]">
                          {s}
                        </button>
                      ))}
                    </div>
                ...
                <div className="p-4 flex justify-between items-start bg-white h-24">
                  <div className="flex flex-col pr-2">
                    <h3 className="text-xs font-medium text-[#1A1A1A] line-clamp-2 leading-tight">{item.name || item.title}</h3>
                    <p className="text-xs text-[#1A1A1A]/60 mt-1.5">{typeof item.price === 'number' ? `₹${item.price.toFixed(2)}` : item.price}</p>
                  </div>
                  <button 
                    onClick={(e) => handleWishlistToggle(e, item)}
                    className={`transition-colors shrink-0 ${wishlist.some(w => w.id === item.id) ? 'text-[#1A1A1A]' : 'text-[#1A1A1A]/40 hover:text-[#1A1A1A]'}`}
      ...
      <section className="py-32 md:py-48 px-6 bg-[#F8F5F1] flex flex-col items-center justify-center text-center w-full">
        <div className="w-full max-w-xl">
          <h3 className="font-serif text-3xl md:text-5xl mb-6 text-[#1A1A1A]">Join the Archive</h3>
          ...
          <form className="flex flex-col md:flex-row w-full gap-4 border-b border-[#1A1A1A]/30 pb-4">
            <input type="email" placeholder="Email Address" className="bg-transparent w-full outline-none text-[#1A1A1A] placeholder:text-gray-400 text-sm" />
            <button type="button" className="text-xs uppercase tracking-widest text-[#1A1A1A] font-medium hover:opacity-60 transition-opacity text-left md:text-right">Subscribe</button>
          </form>
        </div>
      </section>
